    ...
    methods: {
        ...
        update_cameras() {
            for (let i=1; i<=3; ++i) {
                ...
                if (i <= 2) {
                    camera.zoom = 0.3 * (1-ratio) + 0.25 * ratio
                    ...
                    camera.zoom = 0.7 * (1-ratio) + 0.25 * ratio
                    let delta = 10
                    camera.position.set(180, 60 + delta, 1000)
                    camera.updateProjectionMatrix()
                    orbit_control.target.set(0, delta, 0 )
        ...
        left(idx) {
            let ratio = this.time_ratio()
            if (idx < 2) {
                return (idx * 900 - 330) * (1-ratio) + (2 * 900 - 300) * ratio
            }
            return (2.5 * 900 - 300) * (1-ratio) + (3 * 900 - 300) * ratio
        },
    },
    watch: {
        time() {
