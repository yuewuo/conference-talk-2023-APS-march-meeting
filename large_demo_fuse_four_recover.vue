...
export default {
    ...
    methods: {
        ...
        snapshot_idx(idx) {
            let time = this.time
            if (time < padding) return 37
            if (time < padding + single_animate) {
                return this.smooth_animate((time - padding) / single_animate) + 37
            }
            if (time < padding + pause + single_animate) {
                return 38
            }
            if (time < padding + pause + 2 * single_animate) {
                return this.smooth_animate((time - padding - pause - single_animate) / single_animate) + 45
            }
            return 46
        },
